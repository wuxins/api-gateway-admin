package com.zhongan.life.utils;
...
public class GTest {
     ...
     * @param jsonObject
     */
    public static StringBuffer getAllKey(String currentKey, JSONObject jsonObject) {
        StringBuffer stringBuffer = new StringBuffer();
        Iterator<String> keys = jsonObject.keySet().iterator();// jsonObject.keys();
        while (keys.hasNext()) {
            String key = keys.next();
            stringBuffer.append(currentKey + "." + key).append(",");
            if (jsonObject.get(key) instanceof JSONObject) {
                JSONObject innerObject = (JSONObject) jsonObject.get(key);
                stringBuffer.append(getAllKey(currentKey + "." + key, innerObject));
    ...
    public static StringBuffer getAllKey(String currentKey, JSONArray json1) {
        StringBuffer stringBuffer = new StringBuffer();
        if (json1 != null) {
            ...
            while (i1.hasNext()) {
                ...
                if (key instanceof JSONObject) {
                    ...
                    stringBuffer.append(getAllKey(currentKey, innerObject));
    ...
    public static void main(String[] args) {
        flatJsonKey(null, ".", JSON.parse(st1)).forEach(System.out::println);
    }

    private static Set<String> flatJsonKey(String curKey, String flatSeparator, Object object) {
        ...
        if (object instanceof JSONObject) {
            ...
            for (String key : keys) {
                ...
                if (curKey == null) {
                    ...
                    newKey = curKey + flatSeparator + key;
                }
                results.add(newKey);
                Object o = obj.get(key);
                results.addAll(flatJsonKey(newKey, flatSeparator, o));
            }
        } else if (object instanceof JSONArray) {
            JSONArray array = (JSONArray) object;
            for (Object item : array) {
                results.addAll(flatJsonKey(curKey, flatSeparator, item));
            }
        }
        return results;
    }
